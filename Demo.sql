...
EXECUTE AS LOGIN = 'C00002';
EXEC sp_Deposit @AccountID = 'A000000002', @Amount = 1000.00;
REVERT;
GO
...
--Customer can view their record by select directly with the table
...
EXECUTE AS LOGIN = 'C00001';
EXEC sp_Transfer @AccountID = 'A000000001', @ToAccountID = 'A000000002', @Amount = 50.00, @Pin = '123456';
REVERT;
GO

---------------------------------------------------------
SELECT AccountID, CustomerID, AccountType, Balance FROM Account;
SELECT * FROM TransactionRecord ORDER BY TransDate;

-- Bank Officer sees masked Salary, Demo with view own record
EXECUTE AS LOGIN = 'BO0001';
SELECT StaffID, Salary, Phone FROM vw_MyStaffRecord;   -- Salary masked (random)
REVERT;

-- Bank Manager sees real Salary (column-level UNMASK granted)
EXECUTE AS LOGIN = 'BM0001';
SELECT StaffID, Salary FROM vw_AllBankOfficers;   -- Salary real
REVERT;
GO
-- Confirm customer phone is masked and ic is encrypt
EXECUTE AS LOGIN = 'BM0001';
SELECT * FROM vw_AllCustomer;   -- Phone masked
REVERT;
GO